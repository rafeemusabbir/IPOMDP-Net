...
class QMDP:
    ...
    def set_terminals(self, term_state, reward=None):
        """
        term_state is a list of linear terminal state coordinates
        """
        term_state = np.array(term_state, 'i')

        # set reward, only where transition is possible
        if reward is not None:
            for x in range(self.num_action):
                non0ind = (self.T[x][:, term_state]).nonzero()
                # reward for reaching goal from anywhere
                self.R[x][non0ind[0], term_state[non0ind[1]]] = reward

        # make goal terminal: can't move away and all actions reward 0
        for x in range(self.num_action):
            self.T[x][term_state, :] = 0.0
            self.T[x][term_state, term_state] = 1.0
            self.R[x][term_state, term_state] = 0
