...
def dijkstra(graph, start, end=None):
	...
	distances = dict()  # dictionary of final distances
	predecessors = dict()  # dictionary of predecessors
	...
	for v in q:
		...
		for w in graph[v]:
			vw_len = distances[v] + graph[v][w]
			if w in distances:
				if vw_len < distances[w]:
					print("Dijkstra: found better path to already-final vertex")
			elif w not in Q or vw_len < q[w]:
				q[w] = vw_len
				predecessors[w] = v
